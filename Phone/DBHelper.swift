...
class DBHelper
{
    ...
    func insert(newContact: Contact)
    {
        ...
        if sqlite3_prepare_v2(db, insertStatementString, -1, &insertStatement, nil) == SQLITE_OK {
            ...
            sqlite3_bind_int(insertStatement, 5, Int32((newContact.emergencyByPass) ? 1 : 0))
    ...
    func refreshDbFromCatalog() {
        ...
        let deleteStatementStirng = "DELETE FROM contact; DELETE FROM contactAdditionalData;"
        var deleteStatement: OpaquePointer? = nil
        if sqlite3_prepare_v2(db, deleteStatementStirng, -1, &deleteStatement, nil) == SQLITE_OK {
            if sqlite3_step(deleteStatement) == SQLITE_DONE {
